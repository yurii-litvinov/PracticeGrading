...
import { ApprovedMeetingAccessesPanel } from "../components/ApprovedMeetingAccessesPanel";
import { MeetingAccessRequestsPanel } from "../components/MeetingAccessRequestsPanel";

export const calculateFinalMark = (
    mark: number,
    scales: MarkScale[] | undefined,
) => {
...
export function ViewMeetingPage() {
            ...
            getCriteriaGroup(meeting.criteriaGroup.id).then((response) => {
                ...
                setMarks(
                    meeting.studentWorks.map((work: StudentWork) => {
                        const averageMarks = work.averageCriteriaMarks.filter(
                            (item) => item.averageMark !== null,
                        );
    ...
    const handleNotification = (action: string) => {
        console.log(action);
        if (
            action.includes(Actions.Join) ||
            action.includes(Actions.SendMark)
        ) {
            fetchData();
            signalRService.current?.sendNotification(
                `${Actions.Highlight}:${selectedStudentId}`,
            );
        }
    };
    ...
    const handleRowClick = (id: number) => {
        ...
        signalRService.current?.sendNotification(
            `${Actions.Highlight}:${id === selectedStudentId ? null : id}`,
        );
    };
        ...
        <>
            <div className="d-flex flex-column flex-sm-row align-items-start justify-content-end w-100">
                ...
                <div className="d-flex flex-column flex-sm-row justify-content-end w-100">
                    <button
                        type="button"
                        className="btn btn-outline-primary btn-lg mb-2 mb-sm-0 me-sm-2"
                    ...
                    <button
                        type="button"
                        className="btn btn-outline-success btn-lg mb-2 mb-sm-0 me-sm-2"
                    ...
                    <button
                        type="button"
                        className="btn btn-light btn-lg mb-2 mb-sm-0 me-sm-2"
                        id="back"
                        onClick={handleBack}
                    >
                        Назад
            ...
            <div className="d-flex flex-column p-2">
                ...
                <div className="d-flex mb-2 align-items-center">
                    ...
                    <span className="form-control-plaintext w-auto text-wrap">
            ...
            {id && (
                <>
                    <MeetingAccessRequestsPanel meetingId={Number(id)} />

                    <ApprovedMeetingAccessesPanel meetingId={Number(id)} />
                </>
            )}
            <hr className="my-4" />
            ...
            <div>
                                        ...
                                        {meeting.studentWorks.map((work) => (
                                            ...
                                            >
                                                <td style={{ width: "30px" }}>
                                                    ...
                                                    >
                                                        <i
                                                            className="bi bi-arrows-angle-expand fs-5"
                                                            style={{
                                                                color: "#007bff",
                                                            }}
                                                        ></i>
                ...
                <div className="d-flex flex-wrap">
                    <div
                        className="flex-grow-1 pe-4 mb-2"
                        style={{ minWidth: "20em" }}
                    >
                        <h4 className="p-2">Список членов комиссии</h4>
                        <MemberList members={meeting.members}></MemberList>
                    </div>
                        ...
                        <div className="card">
                            <div className="card-body">
                                ...
                                {criteriaGroup?.markScales.length !== 0 ? (
                                    <>
                                        <strong className="card-text p-2 pt-0">
